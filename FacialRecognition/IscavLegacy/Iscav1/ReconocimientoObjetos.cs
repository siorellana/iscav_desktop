...
namespace Emgu.CV
{
    ...
    /// </summary>
    [Serializable]
    public class ReconocimientoObjetos
    {
        ...
        public ReconocimientoObjetos(Image<Gray, Byte>[] images, ref MCvTermCriteria termCrit)
           : this(images, GenerateLabels(images.Length), ref termCrit)
        {
        }

        private static String[] GenerateLabels(int size) // esto genera el nombre
        {
        ...
        /// </summary>
        /// <param name="images">The images used for training, each of them should be the same size. It's recommended the images are histogram normalized</param>
        /// <param name="lista">The labels corresponding to the images</param>
        /// <param name="termCrit">The criteria for recognizer training</param>
        public ReconocimientoObjetos(Image<Gray, Byte>[] images, String[] lista, ref MCvTermCriteria termCrit)
           : this(images, lista, 0, ref termCrit)
        {
        }
        ...
        /// </param>
        /// <param name="termCrit">The criteria for recognizer training</param>
        public ReconocimientoObjetos(Image<Gray, Byte>[] images, String[] lista, double eigenDistanceThreshold, ref MCvTermCriteria termCrit)
        {
            Debug.Assert(images.Length == lista.Length, "The number of images should equals the number of labels");
        ...
        /// </summary>
        /// <param name="listaImagenes">The images used for training </param>
        ...
        public static void CalcEigenObjects(Image<Gray, Byte>[] listaImagenes, ref MCvTermCriteria termCrit, out Image<Gray, Single>[] eigenImages, out Image<Gray, Single> avg)
        {
            IntPtr[] inObjs = Array.ConvertAll<Image<Gray, Byte>, IntPtr>(listaImagenes, delegate (Image<Gray, Byte> img) { return img.Ptr; });

            if (termCrit.max_iter <= 0 || termCrit.max_iter > listaImagenes.Length)
                termCrit.max_iter = listaImagenes.Length;
            int width = listaImagenes[0].Width; // aca se caia antes
            int height = listaImagenes[0].Height;// en estos 2
        ...
        /// <param name="list">The label of the specific image</param>
        public void FindMostSimilarObject(Image<Gray, Byte> image, out int index, out float eigenDistance, out String list)
        {
        ...
        /// </returns>
        public String Reconocerte(Image<Gray, Byte> image)
        {
            int index;
            float eigenDistance;
            String lista;
            FindMostSimilarObject(image, out index, out eigenDistance, out lista);

            return (_eigenDistanceThreshold <= 0 || eigenDistance < _eigenDistanceThreshold) ? _lista[index] : String.Empty;
        }
    }
}